...
현황판 데이터 소스 + 비-CLI 사용자 배려 + 토큰 절약 원칙. (공통 모듈 v2.2.0)

- **공통 함수 `overview()` 신설** (양 스택): 이 PC 전체 솔루션의 생존·모듈 버전·스냅샷 신선도·provides/accepts
  를 한 번에 반환 — **종합 현황판·시스템 모니터링 솔루션의 표준 데이터 소스** (집계는 코드가, 화면은 앱이)
- **비-CLI 사용자 배려**: 스킬이 명령을 시키기 전에 "제가 대신 실행할까요?" 제안 (dev 서버는 백그라운드로),
  quickstart 에 "채팅창에 붙여넣고 실행해줘" 경로 안내
- **설계 4원칙 "알뜰(토큰 절약)"** 확정: Pro 요금제 기준 — 필요한 파일만 읽기, 집계는 코드가, 상시 비용
  측정·관리 (연구 문서: docs/design/token-efficiency.md — 상시 ~1,132tok, 목표 1,500 미만)
