...
스킬 슬림화 — Claude Code 기본 능력과 중복되는 부분을 걷어내고 프레임워크 고유값에 집중. (공통 모듈 변경 없음)

- **스킬 8종 → 6종**: 기본 능력과 겹쳐 고유값이 얇던 2종을 흡수 통합.
  - `/axfm-idea` → `/axfm-new` **0단계**(막연하면 세 질문으로 먼저 정리)로 흡수. 트리거("이런 거 만들고 싶은데" 등)는 new 로 이관.
  - `/axfm-qa` → `/axfm-publish` **1단계**(동작·정합 점검)로 흡수. 실제 구동은 기본 `verify`/`run` 에 위임하고, AXFM 고유 정합(계약 3중 일치·디자인 토큰)만 남김. 트리거("점검", "배포 전 체크" 등)는 publish 로 이관.
- **`/axfm-feature`·`/axfm-debug` 압축**: 일반 구현·디버깅 방법론(Claude 기본) 서술을 덜어내고, 각각 **연동 계약 동기화·디자인 토큰**과 **AXFM 원인 순서표**라는 고유값에 집중.
- 근거: `docs/reviews/2026-07-04-skill-evaluation.md` 의 통합 후보 결론을 실행.

## v0.5.1 — 2026-07-05

디자인 표준 v1.1 — 실측 보고서의 디자인 언어를 기본 디자인으로 승격.

- **보고서·대시보드 컴포넌트 4종 추가**: kpi-card(핵심 숫자 카드) · callout(강조 박스) ·
  bar-chart(수평 막대 — 외부 라이브러리 금지) · table(표 규격). 토큰 54 → 74개
- 기존 솔루션 반영: `/axfm-guide` 가 안내하는 CSS 재수출(export-design) 1회면 됨 — 코드 변경 불필요
