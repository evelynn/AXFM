...
accepts:
  - name: feedback
    writes: ".axfm/data/feedback.json"
...
## 받는 것
- **feedback**: `pushData("lunch-pick","feedback",{...})`로 보내면 `.axfm/data/feedback.json`에 쌓입니다.
```
...
3. 내 솔루션에 `connectors/<상대id>.ts`(또는 `.py`) 생성:
   ...
   - 상대에게 데이터를 보낼 때(accepts) → 상대 프로젝트가 `readShared`로 읽도록, 내가 `writeShared`로 약속된 위치에 내보내는 코드 + 상대에 붙일 안내 블록
...
- **손상 시**: 파싱 실패를 조용히 빈 목록으로 만들지 않는다 — 화면/스킬이 "레지스트리 손상 — /axfm-guide로 복구"를 노출한다.
- id 충돌(같은 id·다른 path): 등록 거부 + id 변경 안내(권장 `{owner}-{name}`).
...
4. 안전 파일명(영숫자·하이픈, 경로 탈출 방지).
5. `ts`: 송신은 오프셋 포함 ISO-8601(초 단위), 수신은 `Z` 접미도 허용 (§4 규칙).
6. 레지스트리 등록/갱신(원자적·BOM 없음·같은 id·다른 경로 거부).
