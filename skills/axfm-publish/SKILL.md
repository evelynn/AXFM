...
1. git clone <주소>
2. cd {폴더}
3. (웹앱) npm install && npm run dev   /   (스크립트) .\start.ps1
4. Claude Code 에서 /axfm-guide 입력  ← 자동으로 레지스트리에 등록되고 연동 준비 완료
※ AXFM 플러그인이 없다면 먼저: /plugin marketplace add <사내 저장소>  →  /plugin install axfm@hansol-axfm
```
