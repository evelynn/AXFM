...
const ov = axfm.overview();
if (!Array.isArray(ov) || ov.length !== 2 || !ov.every((s) => s.alive)) throw new Error("overview 이상: " + JSON.stringify(ov));
console.log("READ:" + env2.data.title + ":stale=" + env2.stale + ":ov=" + ov.length);
`, "utf8");
