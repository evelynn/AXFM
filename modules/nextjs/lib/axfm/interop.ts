...
import { makeEnvelope, validateEnvelope, sanitizeName, STALE_AFTER_MS, type Envelope, type RegistryEntry } from "./types";
...
export function loadInterface(solutionId: string): InterfaceDoc | null {
  ...
  const file = join(entry.path, entry.interface ?? join("axfm", "interface.md"));
  let raw: string;
  try {
